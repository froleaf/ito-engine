...
> **关联节点**：{关键知识节点}
> **更新时间**：{最近更新日期}
> **原文来源**：{sourceRef 路径——指向 inbox/conversations/ 或 inbox/processed/ 中的原始文件}

## 导读

{导读是 deposit 被 Agent 调用时唯一会读的内容，因此必须同时满足两个标准：

**语义自足**：一个从未参与过相关对话的 Agent（甚至另一个用户的 brain）读到这份导读，能完整理解核心洞察，不需要任何外部上下文。具体要求：
- 不使用未定义的内部术语（如"三角色框架"）——要么展开解释，要么用公共学术术语替代
- 不引用对话轮次（如"第一轮对话中"）——导读是知识结构，不是叙事
- 每个关键概念首次出现时用一句话定义

**极致简短**：目标 150-250 字（硬上限 300 字）。每句话必须承载信息，删除所有可删除的修饰语。结构：
- **第一句（核心判断）**：这份内容最重要的一个洞察是什么
- **展开（2-4 个要点）**：支撑核心判断的关键论证，每个要点一句话
- **关联（1-2 句）**：与图谱中已有思考的连接

写完后自检：如果删掉任何一句话，导读的核心信息是否会丢失？如果不会，删掉它。}
```

**重要：deposit 文件只包含元数据头和导读，不内嵌原文。** 原文通过 `sourceRef`/`原文来源` 指向 `inbox/conversations/` 或 `inbox/processed/` 中的存档文件。这避免了同一内容在 deposit 和 conversation 中存两份的冗余，同时控制 token 消耗——Agent 读 deposit 时只需读导读，需要原文时通过 sourceRef 路径读取原始文件。

对于非对话来源的 deposit（如用户口述整理、从 URL 提取），原文存入 `inbox/processed/` 并在 deposit 中通过 sourceRef 指向。
...
| 场景 | 操作 | token 消耗 |
|------|------|-----------|
| Agent 需要了解内容 | 只 Read deposit 文件（即导读） | 低 |
| 用户问"这份讲了什么" | 基于导读回答 | 低 |
| 用户说"把原文给我" | 通过 `sourceRef` 找到原始文件路径，用 `cat` 直接打印输出给用户。**Agent 不读取原文内容**，只负责投递 | 无（AI 不处理） |
| 用户说"帮我分析一下原文" / 明确指示 AI 读原文 | 通过 `sourceRef` Read 原始文件，AI 消耗 token 阅读并分析 | 高 |

**关键区分**：
- "给我原文" = 通过 sourceRef 管道投递，Agent 作为文件搬运工，不读不理解
- "读一下原文" / "分析原文" = Agent 通过 sourceRef 读取原始文件并理解
